...
import { Loader2, List, Copy, Check } from "lucide-react";
...
export function KeyTopicsTab({ document }: { document: DocumentData }) {
  const [copied, setCopied] = useState(false);
  const [isGenerating, setIsGenerating] = useState(false);
  const [topics, setTopics] = useState<string | null>(document.keyTopics || null);
  ...
  const generateTopics = async () => {
    ...
    try {
      ...
      if (res.ok && data.keyTopics) {
        setTopics(data.keyTopics);
      } else {
        setError(data.message || data.error || 'Failed to extract topics');
      }
    } catch (err) {
  ...
  if (!topics && !isGenerating && !error) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center p-6">
        <List className="w-12 h-12 text-primary/60 mb-4" />
        <h3 className="text-lg font-medium mb-2">Key Topics</h3>
        <p className="text-muted-foreground mb-6 text-sm">
          Identify the major topics and concepts discussed in this document.
        </p>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6">
        {isGenerating ? (
          <div className="flex flex-col items-center justify-center py-12 space-y-4">
            ...
            <p className="text-muted-foreground animate-pulse text-sm">Extracting key topics...</p>
          </div>
        ) : error ? (
          ...
          <div className="prose prose-sm dark:prose-invert max-w-none">
            <ReactMarkdown remarkPlugins={[remarkGfm]}>{topics}</ReactMarkdown>
          </div>
        ) : null}
      </div>
    </div>
